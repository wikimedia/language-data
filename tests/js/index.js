...
describe( 'languagedata', () => {
	...
	it( 'regions and groups', () => {
		...
		const territories = languageData.getTerritories();
		assert.ok( typeof territories === 'object' && territories !== null, 'getTerritories() returns an object' );
		...
		assert.ok( territories[ 'RU' ].includes( 'sah' ), 'Sakha is listed for the RU territory via getTerritories()' );
		assert.ok( Object.keys( territories ).length > 0, 'getTerritories() returns a non-empty set of territories' );
		assert.strictEqual( territories[ 'no-such-country' ], undefined, 'An invalid territory code is not present in getTerritories()' );
